...
import { verifyOtp } from "@/Services/actions/verifyOtp"; // API call for OTP verification
...
const RegisterModal = ({ isOpen, onClose, onLoginClick }) => {
  ...
  const handleOtpVerification = async (data) => {
    try {
      const res = await verifyOtp({ email, otp: data.otp, token: otpToken });
...
const OtpVerificationModal = ({
  ...
  return (
    <Modal centered open={isOpen} onCancel={() => setOtpModalVisible(false)} footer={null}>
      <form
        onSubmit={handleSubmit((data) => handleOtpVerification(data))}
        className="flex flex-col gap-4 py-10"
      >
